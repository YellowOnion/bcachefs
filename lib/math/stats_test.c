...
static void stats_basic_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ_MSG(test, s.sum_squares, MAX_SQR, "%llu == %llu, sqrt: %llu == %llu", s.sum_squares, MAX_SQR, int_sqrt64(s.sum_squares), SQRT_U64_MAX);
...
static void stats_fast_divpow2(struct kunit *test)
{
	...
	for (i = 0; i < 100; i++) {
		...
		for (d = 1; d < 32; d++) {
			KUNIT_EXPECT_EQ_MSG(test, abs(fast_divpow2(i, d)), div_u64(i, 1 << d), "%lld %u", i, d);
			KUNIT_EXPECT_EQ_MSG(test, abs(fast_divpow2(-i, d)), div_u64(i, 1 << d), "%lld %u", -i, d);
		}
	}
}
