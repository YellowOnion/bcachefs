			  ...
			  int rw, struct printbuf *err)
{
	...
	if (!BCH_ALLOC_V4_BACKPOINTERS_START(a.v) &&
		...
		pr_buf(err, "invalid backpointers_start");
		return -EINVAL;
	}
...
void bch2_alloc_to_text(struct printbuf *out, struct bch_fs *c, struct bkey_s_c k)
{
	...
	pr_newline(out);
	pr_buf(out, "journal_seq       %llu",	a->journal_seq);
	pr_newline(out);
	pr_buf(out, "need_discard      %llu",	BCH_ALLOC_V4_NEED_DISCARD(a));
	pr_newline(out);
	pr_buf(out, "need_inc_gen      %llu",	BCH_ALLOC_V4_NEED_INC_GEN(a));
	pr_newline(out);
	...
	pr_newline(out);
	pr_buf(out, "io_time[READ]     %llu",	a->io_time[READ]);
	pr_newline(out);
	pr_buf(out, "io_time[WRITE]    %llu",	a->io_time[WRITE]);
	pr_newline(out);
	pr_buf(out, "backpointers:     %llu",	BCH_ALLOC_V4_NR_BACKPOINTERS(a));
	pr_indent_push(out, 2);
